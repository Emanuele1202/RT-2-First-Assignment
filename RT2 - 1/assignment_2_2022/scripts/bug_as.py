...
def clbk_odom(msg):
    ...
    Args:
        msg (Odometry): The message received from the topic.
...
def clbk_laser(msg):
    ...
    Args:
        msg (LaserScan): The message received from the topic.
...
def done():
    """
    Stops the robot and publishes a message to the topic.
    """
    twist_msg = Twist()
...
def planning(goal):
    """
    The main function of the node. This function moves the robot to a desired position and performs wall following.
    It takes a goal position as an argument and returns nothing. The function first sets some global variables and parameters, 
    then enters a loop where it continuously checks the robot's current position and surroundings to determine its next action. 
    It uses feedback to communicate its progress and sends a message when the target is reached or cancelled. 
    The function can be preempted by another goal and will stop executing the current goal if this occurs.
    

    Args:
        goal (Pose): The goal position of the robot.
...
def main():
    """
    The main function of the node. This function initializes the node and sets up the action server.
    """
    time.sleep(2)
    global regions_, position_, desired_position_, state_, yaw_, yaw_error_allowed_
